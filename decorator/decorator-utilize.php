 ...
 * We then have two decorators: CreditCardDecorator and PayPalDecorator.
 *
 * The CreditCardDecorator takes in the credit card number, expiration date, and cvv as constructor parameters.
 ...
 * In the example usage section, we create a Payment object and decorate it first with the CreditCardDecorator and then with the PayPalDecorator. We then call the `
 *
 *
 * In this example, we first create a new Payment object. We then decorate it with a CreditCardDecorator and a PayPalDecorator to allow payments to be processed using a credit card and a PayPal account. We also decorate it with a PaymentLoggerDecorator to log all payment transactions and a PaymentRetryDecorator to automatically retry failed payments up to three times.
...
require_once 'CreditCardDecorator.php';
...
// Decorate the Payment object with a CreditCardDecorator
$payment = new CreditCardDecorator($payment, "1234 5678 9012 3456", "04/25", "123");
$payment->pay(2000);
